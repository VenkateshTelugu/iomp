...
from sunlight_analysis import calculate_sunlight
...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user['password'], password):
            ...
            return jsonify({"success": True, "redirect": url_for('user_home', user_id=user_id)})
        else:
...
def user_home():
    ...
    units_value = get_electricity_units(username)  # Retrieve electricity units (you may replace this with actual logic)
    ...
    # Retrieve the solar details using the user_id
    solar_details = users_collection.find_one({'user_id': user_id})  # Fetch solar details for the user
    if solar_details:
        installment_cost = solar_details.get('installment_cost', 0)  # Get the installment cost, default to 0 if not found
        size = solar_details.get('size', 1)  # Get the solar size, default to 1 if not found
    else:
        installment_cost = 0  # Default value if no solar details are found
        size = 1  # Default value if no solar details are found
    ...
    return render_template('home.html', 
                           ...
                           size=size, 
                           user_id=user_id)  # Pass the necessary data to the template

def get_electricity_units(service_number):
    # Placeholder logic for getting electricity units. Replace with actual logic.
    return "100" 
...
def solar_details():
    ...
    if request.method == 'POST':
        ...
        solar_size = request.form.get('solar_size')
        setup_date = request.form.get('setup_date')
        installment_cost = request.form.get('installment_cost')

        # Validate the data if necessary
        if not solar_size or not setup_date or not installment_cost:
